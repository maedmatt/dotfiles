 ...
 *
 * Commands:
 *   /md-log <name>      — Link <name>.md in the default learning folder, creating it if missing.
 *   /md-log <filepath>  — Link an existing markdown file (any arg containing "/"); backfills the session.
 ...
 *
 * Ported from amosblomqvist/learn. Local change: bare names resolve into
 * DEFAULT_LOG_DIR and are auto-created; explicit paths keep the upstream
 * must-exist behavior.
 */
...
const DEFAULT_LOG_DIR = path.join(os.homedir(), "Documents", "01 Projects", "matt-second-brain", "learning");
...
export default function mdLog(pi: ExtensionAPI) {
	...
	pi.registerCommand("md-log", {
		...
		handler: async (args, ctx: any) => {
			...
			// Bare names (no "/") are lesson logs: they live in DEFAULT_LOG_DIR and
			// are created on first link. Explicit paths link into an existing note
			// only — never created, so a typo'd path can't scatter files around.
			let resolved: string;
			if (path.isAbsolute(filepath) || filepath.includes("/")) {
				...
				resolved = path.join(DEFAULT_LOG_DIR, name);
				if (!fs.existsSync(resolved)) {
