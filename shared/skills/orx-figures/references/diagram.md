...
and the same palette the plots use. It lives in this skill's `assets/`
directory, beside the `SKILL.md`. Copy it as the head of the figure's `.tex`:

```sh
mkdir -p figs && cp <skill dir>/assets/orx-tikz-preamble.tex figs/method.tex
```

`figs/method.tex` compiles to `figs/method.pdf` next to it, ready for
`\includegraphics`. Do not run the compiler yourself unless asked: hand over
the `.tex` and let the user's editor tooling build it, or ask first.
...
| `\tikzexternalize`, `pgfplots` externalization, `minted` | Avoid: they need shell-escape, which arXiv and most default builds disable |
