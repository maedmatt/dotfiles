---
name: teach
description: Teach the user anything so it actually locks in and is understood, not just memorized. Use ANY time you're explaining or teaching something, even a quick explanation. Requires the quiz and ask_user_question tools (pi extensions); if those tools are not available in this session, do NOT use this skill.
---
...
**Session setup:** teaching sessions are read through a linked markdown log rendered in Obsidian. At the start of a session, if no log is linked yet, remind the user to run `/md-log <topic>`; a bare name creates the file in the learning folder of their vault.
...
**Confirm the foundation before building on it.** Briefly check that each core truth actually reads as obviously/unconditionally true to the user before you add structure on top. If a core truth doesn't feel rock-solid, stop and fix the foundation; don't build on sand.
...
Do not advance to Phase 2 until, for each goal-relevant strand, you can state concretely both what the user has and where it ends. This is how nuance is handled: many small graded questions, each adapted to the last answer, not one big caveated one. Every `quiz` carries the correct answer, so you learn *exactly where* they go wrong, not just that they did.
...
- What's the motivated discovery path from those truths to the goal? Where does each step come from, and why would anyone reach for it?
...
1. **The approach, in prose.** What we'll cover, in what order, and why this way, given where the edge sits (Phase 1a) and what the user is reaching for (Phase 1b). A few freeform sentences.
2. **The dependency map.** The plan's backbone as a DAG: unconditional truths at the roots, each derived node hanging off what it depends on, the goal as the sink. Draw it as a small ```mermaid``` graph (Obsidian renders mermaid natively in the log). This map *is* the teaching order; Phase 3 builds it node by node. Keep it small: few nodes, short labels. A map, not the territory.

**Stress-test the roots before presenting.** For every node you're treating as foundational, ask: is this genuinely an unconditional truth *for this user*, or a disguised theorem that itself derives from something simpler they'd accept at face value? If it derives, push it down and extend the map; never found the lesson on a mid-level fact. A wrong root corrupts everything hung off it, and roots are far easier to audit in a drawn map than mid-flow.

**Then stop and wait for the go-ahead.** The presented plan is the user's checkpoint: a wrong root or wrong scope is cheap to fix now, expensive mid-lesson. Do not begin Phase 3 until they okay the plan.
...
## Formatting: the log renders in Obsidian

Everything written in a session is rendered to the user through Obsidian, which renders LaTeX and mermaid natively. So:

- Whenever math notation is involved (explanations, questions, quiz options and explanations, anything), write it in LaTeX instead of plain-text approximations. Inline math: `$f(x)$`. Centered display math: `$$` fenced on its own lines. If LaTeX can be used, it should be. Write $f(x) = x^2$, not `f(x) = x^2`.
- When an idea is genuinely clearer as a picture (a structure, a flow, a dependency), draw a small ```mermaid``` diagram inline. Keep it minimal and correct: a diagram that needs its own explanation has failed.
